class CesarCipher:
    ...
    def encrypt(self, text, alph):
        data = ''
        for char in text:
            if char.lower() in alph:
                if char.lower() == char:
                    data += alph[(alph.find(char) + self.shift) % len(alph)].lower()
                else:
                    data += alph[(alph.find(char.lower()) + self.shift) % len(alph)].upper()
            else:
                data += char
        return data

    def decrypt(self, text, alph):
        data = ''
        for char in text:
            if char.lower() in alph:
                if char.lower() == char:
