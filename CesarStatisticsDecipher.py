...
class CesarStatisticsDecipher:
    ...
    def collect_statistics_from_text(self, text, alph):
    ...
    def generate_shifted_alph(self, shift, alph):
        cyp = CesarCipher(shift)
        return cyp.encrypt(alphabets[alph][0], alphabets[alph][0])

    def find_opt_step(self, text, alph):
        ...
        for i in range(0, len(alphabets[alph][0])):
            ...
            standard_deviation = (sum((statistics_data_lang[char] - temp_dictionary[char]) ** 2 for char in new_alpha) / len(new_alpha)) ** 0.5
